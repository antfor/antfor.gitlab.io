...
export default function Result({ datasets, total, index, dataPoints }: props) {
    ...
    const acc = datasets.filter((dataset) => !dataset.interest);
    const interest = datasets.filter((dataset) => dataset.interest && !dataset.interestOnInterest);
    const interestInterest = datasets.filter((dataset) => dataset.interest && dataset.interestOnInterest);
        ...
        <Col>
            <Row className="row justify-content-between">
                <Col xl={9} className='col-xl'>
                    ...
                    {Brake(interestInterest)}
                    <Row>
                        {interestInterest.map((dataset) => Paragraph(dataset, index, total))}
                    </Row>
                    {Brake(interest)}
                    <Row>
                        {interest.map((dataset) => Paragraph(dataset, index, total))}
                    </Row>
                    {Brake(acc)}
                    <Row>
