...
class FractalFactory {
  ...
  treeTernary(scale: Vec3 = [1, 1, 1], step = 1.0, dir: Vec3 = [0, 1, 0], lr: number, vr: number, d1: number, d2: number, a: number, T: Vec3, e: number) {
    ...
    const rA = new Rule("A", ['F', 50, '[', '&', a,'!',vr, 'F', 50, 'A', ']', '/', d1, '[', '&', a,'!',vr, 'F', 50, 'A', ']', '/', d2, '[', '&', a,'!',vr, 'F', 50, 'A', ']'], () => { });
    const rF = new Rule("F", ((l: number) => ['F', l * lr]) as succ, ((s: State, val: number) => { tf(T, e, forwardColorVal, s, val) }) as func, true);
    const rf = new Rule("f", ((l: number) => ['f', l * lr]) as succ, ((s: State, val: number) => { forwardColorVal(s, val) }) as func, true);
    const rw = new Rule("!", ((w: number) => ['!', w * vr]) as succ, ((s: State, val: number) => {width(s, val)}) as func, true);
    const rs = new Rule("[", undefined, push);
    const rl = new Rule("]", undefined, pop);
    const rp = new Rule("/", undefined, ((s: State, a: number) => { roll(s, -a) }) as func, true);
    const rm = new Rule("&", undefined, ((s: State, a: number) => { pitch(s, a) }) as func, true);
    const rB = new Rule("B", undefined, (s: State) => { color(s, [140 / 255, 80 / 255, 60 / 255]) });
 
    return new Fractal(axiom, 0, scale, step, dir, rA, rF, rf, rw, rs, rl, rp, rm, rB);
  }
  ...
  ternaryTreeA(scale: Vec3 = [1, 1, 1], step = 1.0, dir: Vec3 = [0, 1, 0]) {
    ...
    ternaryTreeB(scale: Vec3 = [1, 1, 1], step = 1.0, dir: Vec3 = [0, 1, 0]) {
    const d1 = 137.50;
    const d2 = 137.50;
    const a = 18.95;
    const lr = 1.109;
  ...
  ternaryTreeC(scale: Vec3 = [1, 1, 1], step = 1.0, dir: Vec3 = [0, 1, 0]) {
    const d1 = 112.50;
    const d2 = 157.50;
    const a = 22.50;
    const lr = 1.0790;
    const vr = 1.732;
    const T: Vec3 = [-0.02, -1, 0];
    const e = 0.27;
  ...
  testTropism(scale: Vec3 = [1,1,1], step = 1.0, dir: Vec3 = [0,1,0]) {
    const e = 0.4;
    const T: Vec3= [-0.61,0.77,-0.19]
    
    // Axiom: one parent branch
    const axiom = ['F','A'];
    

    // Rule A: forward with tropism
    const rA = new Rule("A","AA",(s: State) => { tf(T, e, forwardColor, s, 1);});

    // Rule F: normal forward
    const rF = new Rule("F", "F",((s: State) => { forwardColor(s); }));

    return new Fractal(axiom, 0, scale, step, dir, rA, rF);
  }
