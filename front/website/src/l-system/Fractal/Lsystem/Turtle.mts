...
function tropismVal(T: Vec3, e: number, f: (state: State, val: number) => void, state: State, val: number) {
  ...
  const T_local = m4.transformDirection(m, T);
  v3.normalize(T_local, T_local); 
  //v3.normalize(T, T); 

  const axis = v3.cross(H, T_local);
  const len = v3.length(axis);
...
export function getYAxis(m: m4.Mat4, dst?: v3.Vec3): v3.Vec3 {
  const out = dst ?? v3.create();
  out[0] = m[4];
  out[1] = m[5];
  out[2] = m[6];
  return out;
