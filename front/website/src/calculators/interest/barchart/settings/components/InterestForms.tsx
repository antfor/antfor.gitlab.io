...
import { Value, interestSettings } from '../defaultSettings.js';
import { formatValue } from '../../utils/parse.mjs';
...
type keyInterest = keyof interestSettings;
interface propsInterestForm {
    settings: interestSettings,
    keyInterest: keyInterest,
    decimals: number,
    style: string,
    change: (k: keyInterest, v: string) => void,
}
export function InterestForms({ settings, keyInterest, decimals, style, change }: propsInterestForm) {

    const value = settings[keyInterest] as Value;
    const props = { value, decimals, style, change: (v: string) => { change(keyInterest, formatValue(v, decimals)) } };
